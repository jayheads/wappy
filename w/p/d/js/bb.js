(function(root, factory) { //     Backbone.js 1.1.2
    ...
    var previousBackbone = root.Backbone // Save the previous value of the `Backbone` variable, so that it can be restored later on, if `noConflict` is used.
    var array=[], push = array.push, slice = array.slice, splice = array.splice // Create local references to array methods we'll want to use later.


    Backbone.VERSION = '1.1.2'// Current version of the library. Keep in sync with `package.json`.
    Backbone.$=$// For Backbone's purposes, jQuery, Zepto, Ender, or My Library (kidding) owns the `$` variable.
    ...
    Backbone.emulateHTTP = false // Turn on `emulateHTTP` to support legacy HTTP servers. Setting this option will fake `"PATCH"`, `"PUT"` and `"DELETE"` requests via the `_method` parameter and set a `X-Http-Method-Override` header.
    ...
    var Events = Backbone.Events = {
        ...
        off: function(name, callback, context) {var retain, ev, events, names, i, l, j, k
    ...
    _.extend(Model.prototype, Events, {
        ...
        set: function(key, val, options) {
            ...
            if (typeof key === 'object') {attrs = key; options = val} else {
                (attrs = {})[key] = val}
        ...
        destroy: function(options) {
            ...
            if (this.isNew()) {options.success(); return false}
        ...
        clone: function() {return new this.constructor(this.attributes)},// Create a new model with identical attributes to this one.

        // A model is new if it has never been saved to the server, and lacks an id.
        isNew: function() {return !this.has(this.idAttribute)},


        isValid: function(options) {return this._validate({}, _.extend(options || {}, { validate: true }))},// Check if the model is currently in a valid state.
    ...
    // If a `comparator` is specified, the Collection will maintain its models in sort order, as they're added and removed.
    ...
    _.extend(Collection.prototype, Events, {
        model: Model, // The default model for a collection is just a **Backbone.Model**. This should be overridden in most cases.
        initialize: function(){},
        toJSON: function(options){return this.map(function(model){ return model.toJSON(options)})},// The JSON representation of a Collection is an array of the models' attributes.
        sync: function() { return Backbone.sync.apply(this, arguments)}, // Proxy `Backbone.sync` by default.
        add: function(models, options) {return this.set(models, _.extend({merge: false}, options, addOptions))}, // Add a model, or list of models to the set.
        ...
        remove: function(models, options) {
            ...
            for (i = 0, l = models.length; i < l; i++) {
                ...
                this.length--;
                if (!options.silent) {options.index = index; model.trigger('remove', model, this, options)}
        ...
        set: function(models, options) {
            ...
            for (i = 0, l = models.length; i < l; i++) {
                attrs = models[i] || {};
                if (attrs instanceof Model){id = model = attrs}
                else {id = attrs[targetModel.prototype.idAttribute || 'id']}
        ...
        push: function(model, options) {return this.add(model, _.extend({at: this.length}, options))},// Add a model to the end of the collection.
        pop: function(options){var model = this.at(this.length - 1); this.remove(model, options); return model},// Remove a model from the end of the collection.
        unshift: function(model, options) {return this.add(model, _.extend({at: 0}, options))}, // Add a model to the beginning of the collection.
        shift: function(options) {var model = this.at(0); this.remove(model, options); return model}, // Remove a model from the beginning of the collection.
        slice: function() {return slice.apply(this.models, arguments)}, // Slice out a sub-array of models from the collection.
    ...
    var View = Backbone.View = function(options) {options || (options = {})
    ...
    _.extend(View.prototype, Events, {// Set up all inheritable **Backbone.View** properties and methods.
        ...
        render:function(){return this}, // **render** is the core function that your view should override, in order to populate its element (`this.el`), with the appropriate HTML. The convention is for **render** to always return `this`.


        remove: function(){this.$el.remove(); this.stopListening(); return this}, // Remove this view by taking the element out of the DOM, and removing any applicable Backbone.Events listeners.
        ...
        delegateEvents: function(events) {
            ...
            for (var key in events) {
                ...
                if (!method) continue
                var match = key.match(delegateEventSplitter= /^(\S+)\s*(.*)$/)
    ...
    Backbone.sync = function(method, model, options) {
        ...
        if (params.type === 'PATCH' && noXhrPatch) {
            params.xhr = function() {return new ActiveXObject("Microsoft.XMLHTTP")}}
    ...
    _.extend(Router.prototype, Events, {
        ...
        route: function(route, name, callback) {
            ...
            if (_.isFunction(name)){callback = name; name = ''}
        ...
        execute: function(callback, args) {if (callback) callback.apply(this, args)},

        // Simple proxy to `Backbone.history` to save a fragment into the history.
        navigate: function(fragment, options) {
            Backbone.history.navigate(fragment, options); return this},
    ...
    var isExplorer = /msie [\w.]+/;// Cached regex for detecting MSIE.
    var trailingSlash = /\/$/;// Cached regex for removing a trailing slash.
    var pathStripper = /#.*$/; // Cached regex for stripping urls of hash.


    History.started = false;// Has the history handling already been started?

    // Set up all inheritable **Backbone.History** properties and methods.
    _.extend(History.prototype, Events, {
        interval: 50,// The default interval to poll for hash changes, if necessary, is twenty times a second.
        atRoot: function(){return this.location.pathname.replace(/[^\/]$/, '$&/') === this.root},// Are we at the app root?
    ...
    var extend = function(protoProps, staticProps) {var parent = this, child
        ...
        // (the "constructor" property in your `extend` definition), or defaulted by us to simply call the parent's constructor.
    ...
    Model.extend = Collection.extend = Router.extend = View.extend = History.extend = extend // Set up inheritance for the model, collection, router, view and history.


    var urlError=function(){throw new Error('A "url" property or function must be specified')}// Throw an error when a URL is needed, and none is supplied.
